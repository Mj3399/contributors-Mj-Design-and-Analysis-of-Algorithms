...
template <class T> class BaseFieldAdapter {
public:
...
template <class T, class C> class FieldAdapter : public BaseFieldAdapter<T> {
public:
...
template <class T> class AbstractTableAdapter : public ITable {
public:
  typedef T DataType;
...
template <class T> class RecordAdapter : public AbstractTableAdapter<T> {
public:
  typedef T DataType;
...
template <class T> class TableAdapter : public AbstractTableAdapter<T> {
public:
  typedef T DataType;
...
void Encode(int value, std::string &result) { IntToStrHelper(value, result); }
...
bool Parse(StringSegment segment, bool &result) {
  ...
  if (segment.Match("false") || segment.Match("no"))
    return true;
...
template <class T> void AddDefaultProblemColumns(TableAdapter<T> &prAdapter) {
...
bool StringSegment::Match(const char *pattern,
  ...
  while ((pattern[iPattern] != 0) && (iStr < iEnd) && bMatch) {
    ...
    if (!bEqualChars) {
      if (!bCaseSensitive) {
        bEqualChars =
            (::tolower(pattern[iPattern]) == ::tolower(str[iPattern]));
      }
...
template <class T> void AbstractTableAdapter<T>::SetDefaultValues(size_t row) {
  assert(row < RowCount());
...
template <class T> size_t AbstractTableAdapter<T>::ColumnCount() const {
  return columnSpecs.size();
}

template <class T> bool TableAdapter<T>::NewRow(size_t &row) {
  data.emplace_back();
  row = data.size() - 1;
...
template <class T> size_t TableAdapter<T>::RowCount() const {
  return data.size();
}

template <class T> bool TableAdapter<T>::IsFixedSize() const { return false; }

template <class T> T &TableAdapter<T>::GetRecord(size_t i) {
  assert(i < data.size());
  return data[i];
}

template <class T> const T &TableAdapter<T>::GetRecord(size_t i) const {
  assert(i < data.size());
  return data[i];
}

template <class T> bool RecordAdapter<T>::NewRow(size_t &row) { return false; }

template <class T> bool RecordAdapter<T>::IsFixedSize() const { return true; }

template <class T> size_t RecordAdapter<T>::RowCount() const { return 1; }

template <class T> T &RecordAdapter<T>::GetRecord(size_t i) {
  assert(i == 0);
  return data;
}

template <class T> const T &RecordAdapter<T>::GetRecord(size_t i) const {
  assert(i == 0);
  return data;
}

BasicYamlParser::BasicYamlParser(ITable *pHeader, ITable *pTable)
    : AbstractLineParser(), pHeader(pHeader), pTable(pTable),
      isHeaderSection(true) {
...
void WriteRecordToStream(std::ostream &out, ITable *table, size_t row,
...
void WriteTableToStream(std::ostream &out, ITable *table,
...
void PreprocessProblemSet(int problem_set_id, std::vector<T> &problems,
  ...
  ExitIfConditionFails(header.problem_count == (int)problems.size(),
